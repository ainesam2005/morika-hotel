import { Utensils, Waves, Dumbbell, Car, Heart, Coffee, Wifi, Clock, Star, Phone } from 'lucide-react';
...
const services = [
  ...
  },
  {
    ...
    description: 'Our state-of-the-art fitness center is open around the clock, equipped with the latest cardiovascular and strength training equipment. Personal trainers are available on request to keep your routine on track.',
  ...
  },
  {
    ...
    description: 'Step into our contemporary grand lobby — a welcoming space of warmth and elegance. Our dedicated concierge team is at your service 24 hours a day, ready to arrange anything your stay requires.',
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-24">
        {services.map((svc, i) => {
          ...
          return (
            <div
              key={svc.title}
              className={`grid grid-cols-1 md:grid-cols-2 gap-0 rounded-2xl overflow-hidden border border-navy-lighter ${
                !imageLeft ? 'md:flex md:flex-row-reverse' : ''
              }`}
            >
              {/* Image */}
              <div className="relative h-72 md:h-auto min-h-[300px] overflow-hidden">
                <img
                  src={svc.image}
                  alt={svc.title}
                  className={`w-full h-full object-cover ${svc.imgPosition} hover:scale-105 transition-transform duration-700`}
                />
      ...
      <div className="max-w-7xl mx-auto px-4 mt-24">
        <div className="relative rounded-2xl overflow-hidden">
          ...
          <div className="absolute inset-0 bg-navy/75 flex flex-col items-center justify-center text-center px-6">
            ...
            <Link to="/rooms" className="btn-gold inline-block">Browse Rooms</Link>
          </div>
        </div>
      </div>
    </div>
